...
namespace Chap15App
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine($"{item.Name}, {item.Height}cm, {item.InchHeight}inch"); //객체 출력
            ...
            var resProfiles2 = from item in profiles
                               where item.Height < 175
                               orderby item.Height ascending //descending
                               select item.Height; //item.name
            foreach (var item in resProfiles2)
            {
                Console.WriteLine($"{item}"); //string 형식이라 item
            }
            ...
            var joinProfiles = from p in profiles
                               join d in products
                               on p.Name equals d.Star //p.name 과 d.star과 같을 때 join
                               select new
                               {
                                   Name = p.Name,
                                   Work = d.Title,
